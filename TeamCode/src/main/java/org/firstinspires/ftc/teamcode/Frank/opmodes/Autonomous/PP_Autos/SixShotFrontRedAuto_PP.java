...
@Autonomous(name = "🔴 6 Shot FRONT", group = "(1) PP Autonomous (Red)")
public class SixShotFrontRedAuto_PP extends OpMode {
    ...
    private final Pose INTAKE_CONTROL = new Pose(88, 82);
    private final Pose INTAKE_POS = new Pose(128, 83);
    private final Pose PARK_POS = new Pose(89, 120);
    ...
    public void init() {
        ...
        path4 = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(Math.toRadians(SHOOTING_ANGLE), Math.toRadians(-90))
                .build();

        // Lower shooter to starting position
        robot.shooter.setTiltAngle(robot.shooter.MIN);

        telemetry.addLine("Ready - 6 Shot Front Red (Non-Blocking)");
        telemetry.update();
    }
