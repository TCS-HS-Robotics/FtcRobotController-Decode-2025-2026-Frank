...
@Autonomous(name = "\uD83D\uDD34 9 Shot BACK", group = "(1) PP Autonomous (Red)")
public class NineShotBackRedAuto_PP extends OpMode {
    ...
    private final Pose START_POSE = new Pose(87.043, 8.554, Math.toRadians(90));
    private final Pose SCORING_POS = new Pose(87.043, 14.949);

    // Row 1 (first pickup) - mirrored from blue (54, 36) and (9, 36)
    private final Pose APPROACH_1 = new Pose(90, 36);
    private final Pose ARTIFACT_1 = new Pose(135, 36);

    // Row 2 (second pickup) - mirrored from blue (57, 60) and (9, 60)
    private final Pose APPROACH_2 = new Pose(87, 60);
    private final Pose ARTIFACT_2 = new Pose(135, 60);
    ...
    private enum AutoState {
        ...
        // Second artifact row (row 2) - ends after pickup
        PATH_5,
        START_INTAKE_2,
        PATH_6,
        EXTRA_INTAKE_2,
    ...
    public void init() {
        ...
        // Build all paths (mirrored headings: 180° → 0°)
        ...
        // Path 2: Scoring to approach row 1
        ...
        // Path 3: Approach to artifact row 1 (intake)
        path3 = follower.pathBuilder()
                .addPath(new BezierLine(APPROACH_1, ARTIFACT_1))
                .setConstantHeadingInterpolation(Math.toRadians(0))
                .build();

        // Path 4: Artifact row 1 back to scoring
        ...
        // Path 5: Scoring to approach row 2
        path5 = follower.pathBuilder()
                .addPath(new BezierLine(SCORING_POS, APPROACH_2))
                .setLinearHeadingInterpolation(Math.toRadians(SHOOTING_ANGLE), Math.toRadians(0))
                .build();

        // Path 6: Approach to artifact row 2 (intake) - ends here
        path6 = follower.pathBuilder()
                .addPath(new BezierLine(APPROACH_2, ARTIFACT_2))
                .setConstantHeadingInterpolation(Math.toRadians(0))
                .build();
        ...
        telemetry.addLine("Ready - 9 Shot Back Red");
        telemetry.update();
    }
    ...
    private void updateDrawing() {
        ...
        drawPathChain(path6, plannedPathStyle);
